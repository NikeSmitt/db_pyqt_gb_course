...
from db_handler import User, UserHistory

# получаем логгера
from discriptors.port import Port
from crud import get_multy, add_user, add_login_history, get_user_by_name, \
    get_user_contacts, add_contact, delete_contact
from session import SessionLocal
from metaClasses.serverVerifier import ServerChecked
...
class Server(ServerChecked):
    ...
    def save_login_to_db(self, client_name):
        """
        Сохраняем пользователя и время входа в БД
        :param client_name: Имя клиента str
        :return: None
        """
        user = add_user(client_name, SessionLocal)
        add_login_history(user)

        pprint(get_multy(User))
        pprint(get_multy(UserHistory))

    def is_contacts_request(self, message: dict) -> bool:
        """
        Проверяем, что это сервисное сообщение на получение списка контактов
        :param message: dict
        :return: bool
        """
    ...
    def add_delete_contact_request(self, message: dict) -> bool:
        """
        Проверяем, что это сервисное сообщение на сохрание пользователя в списке контактов
        :param message:
        :return:
        """
        if message.get(ACTION) == 'add_contact':
            name = message.get(USER)
            name_to_save = message.get('user_name')
            if not (name and name_to_save):
                raise AttributeError('Ошибка при добавлении контакта')
            add_contact(name, name_to_save)
            return True
            ...
            if not (name and name_to_save):
                raise AttributeError('Ошибка при добавлении контакта')
            delete_contact(name, name_to_save)
            return True
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                if self.clients:
                    receive_data_clients, send_data_clients, error_clients = select.select(self.clients, self.clients,
                                                                                           [], 0)
            except OSError:
                pass
                ...
                try:
                    ...
                    if self.is_presence_message(client_message):
                        if self.check_username_is_free(client_message, self.chat_usernames):
                            ...
                            self.save_login_to_db(client_message[USER][ACCOUNT_NAME])
                    ...
                    elif self.is_contacts_request(client_message):
                        user_name = client_message.get(USER)
                        if user_name is None:
                            raise AttributeError('Неверно сформирован запрос для списка контактов')
                        user_contacts = get_user_contacts(user_name)
                        message = {
                            RESPONSE: 202,
                            'alert': user_contacts
                        }
                        utils.send_message(client_with_message, message)

                    elif self.is_regular_message(client_message):
                        self.messages.append(client_message)

                    elif self.add_delete_contact_request(client_message):
                        pass
            ...
            for message in self.messages:
                try:
                    print(f'{message=}')
                    if message[DESTINATION] in self.chat_usernames and self.chat_usernames[
                        message[DESTINATION]] in send_data_clients:
                        utils.send_message(self.chat_usernames[message[DESTINATION]], message)
                        server_logger.info(f'Отправлено сообщение пользователю {message[DESTINATION]} '
                                           f'от пользователя {message[SENDER]}.')
                    elif message[DESTINATION] in self.chat_usernames and self.chat_usernames[
                        message[DESTINATION]] not in send_data_clients:
