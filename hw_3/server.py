...
class Server(ServerChecked):
    ...
    def __init__(self, listen_address: str, listen_port: int, db_engine):
    ...
    def save_login_to_db(self, client_name):
        ...
        with Session(self.db_engine) as session:
            ...
            user = session.query(User).filter_by(nickname=client_name).first()
            if user:
                session.add(UserHistory(user.id))
                session.commit()
            else:
                session.add(User(client_name))
                session.commit()
            ...
            pprint(session.query(UserHistory).all())
...
if __name__ == "__main__":
    ...
    engine = create_engine(f'sqlite:///main.db', echo=True)

    server = Server(listen_address, listen_port, engine)
    server.run()
