...
from common.variables import DEFAULT_PORT, DEFAULT_IP_ADDRESS, ENCODING, ACTION, PRESENCE, \
    ACCOUNT_NAME, MESSAGE_TEXT, SENDER, MESSAGE, DESTINATION, TIME, EXIT, USER, RESPONSE
from decos import log

# получаем логгера
import crud
from session import ClientSessionLocal
...
class Client(ClientChecked):
    ...
    def getting_messages_from_server(self):
        ...
        while True:
            try:
                ...
                #     print(f'Получен ответ от сервера: response : {message.get(RESPONSE)}')
    ...
    def print_help(self):
        ...
        print('add_contact - сохранить контакт пользователя')
        print('del_contact - удалить контакт пользователя')
    ...
    def user_interactive(self):
        ...
        while True:
            ...
            if user_command == 'message':
                ...
                crud.save_message(self.name, message[MESSAGE_TEXT], ClientSessionLocal)
                try:
                ...
                self.add_contact(self.client_socket)

            elif user_command == 'del_contact':
                self.del_contact(self.client_socket)
    ...
    @log
    def add_contact(self, sock: socket):
        """
        Запрос на сохрание контакта
        :return:
        """
        name_to_save = input('Введите имя контакта -> ')
        message = {
            ACTION: 'add_contact',
        ...
        crud.add_user(name_to_save, ClientSessionLocal)

    @log
    def del_contact(self, sock: socket):
        """
        Запрос на удаление контакта
        :param sock:
        :return:
        """
        name_to_del = input('Введите имя контакта -> ')
        message = {
            ACTION: 'del_contact',
            'user_name': name_to_del,
            TIME: datetime.datetime.now().ctime(),
            USER: self.name
        }
        utils.send_message(sock, message)
    ...
    @log
    def request_contacts(self, sock: socket):
        message = {
            ACTION: "get_contacts",
            TIME: datetime.datetime.now().ctime(),
            USER: self.name
    ...
    @log
    def parse_contacts(self, message: dict) -> list:
        if message.get('response'):
            if message['response'] != 202:
                raise ConnectionError('Что-то не так со списком контактов')
            return message.get('alert')
    ...
    def run(self):
        try:
            ...
            self.request_contacts(self.client_socket)
            response = utils.get_message(self.client_socket)
